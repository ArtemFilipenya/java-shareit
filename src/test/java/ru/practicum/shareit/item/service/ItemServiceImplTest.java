...
class ItemServiceImplTest {
    ...
    @BeforeEach
    void initialize() {
        userDto = userService.save(new UserDto(null, "Joe", "joe@mail.com"));
        itemDto = itemService.save(new ItemDto(null, "Pen", "Blue Pen", true, null),
                null, userDto.getId());
    }
    ...
    void getItemsByRequestIdEmptyResultTest() {
        UserDto requester = userService.save(
                new UserDto(null, "Zoe", "zoe@mail.com"));
        ItemRequestDto itemRequestDto = itemRequestService.save(
                new ItemRequestDto(null, "I need it", requester.getId(), now(), of()), requester.getId());
        List<ItemDto> itemsByRequestId = itemService.getItemsByRequestId(itemRequestDto.getId());
        List<Item> itemsByRequest = entityManager.createQuery(
                        "SELECT item " +
                                "FROM Item item " +
    ...
    void getAllCommentsIdTest() {
        saveCommentDto("Hello there", new UserDto(15L, "Douglas", "douglas@mail.com"));
